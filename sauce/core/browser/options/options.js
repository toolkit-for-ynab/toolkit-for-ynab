...
function saveCheckboxOption(elementId) {
  ...
  if (element) {
    return setToolkitSetting(elementId, element.checked);
  }
...
function saveSelectOption(elementId) {
  ...
  if (select) {
    return setToolkitSetting(elementId, select.options[select.selectedIndex].value);
  }
...
function restoreCheckboxOption(elementId) {
  return new Promise(function (resolve) {
    ...
    if (element) {
      getToolkitSetting(elementId).then(function (value) {
...
function restoreSelectOption(elementId) {
  return new Promise(function (resolve) {
    ...
    if (select) {
      getToolkitSetting(elementId).then(function (data) {
        data = data || 0;
...
function updateToolkitLogo() {
  getToolkitSetting('DisableToolkit').then((isDisabled) => {
    if (isDisabled) {
...
function toggleToolkit() {
  getToolkitSetting('DisableToolkit').then((isDisabled) => {
    setToolkitSetting('DisableToolkit', !isDisabled);
    updateToolkitLogo();
  });
}
...
function setLogo(status = APP_STATES.normal) {
  const logos = {
    normal: '../../assets/images/logos/toolkitforynab-logo-200.png',
    warning: '../../assets/images/logos/toolkitforynab-logo-200-warning.png',
    disabled: '../../assets/images/logos/toolkitforynab-logo-200-disabled.png'
  };

  jq('#logo').attr('src', logos[status]);
}
...
jq(function () {
  ...
  getToolkitSetting('options.dark-mode').then(function (data) {
    applyDarkMode(data);
  ...
  // set version number
  jq('.toolkit-version').text('v ' + getExtensionInfo().version);
