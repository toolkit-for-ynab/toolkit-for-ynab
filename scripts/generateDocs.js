...
const allToolkitSettings = JSON.parse(fs.readFileSync(path.join(__dirname, 'settings.json'), 'utf8'));
...
let docOutput = '<!-- THIS FILE IS GENERATED THERE IS NO NEED TO ADD YOUR FEATURE TO THIS LIST -->\n# List of YNAB Toolkit Features\n\n';
docOutput += docSettingsOrder.map(({ key, name }) => {
  let settingSection = `<details><summary>${name} (Click to Expand/Collapse)</summary>\n`;

  const sectionSettings = settingsMap.get(key);
  settingSection += sectionSettings.map(({ description, title }) => {
    return `\n## ${title}\n${description}`;
  }).join('\n');
