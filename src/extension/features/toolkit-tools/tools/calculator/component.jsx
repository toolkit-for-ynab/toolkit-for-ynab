...
export class CalculatorComponent extends React.Component {
  static propTypes = {
    ...
    // allReportableTransactions: PropTypes.array.isRquired,
  };
  ...
  render() {
    return (
      <div className="tk-flex tk-flex-column tk-flex-grow">
        ...
        <div className="tk-highcharts-tool-container" id="tk-net-worth-chart" />
      </div>
    );
  }
  ...
  _renderReport = () => {
    const _this = this;
    const { labels, debts, assets, netWorths } = this.state.toolData;
      ...
      series: [
        {
          id: 'debts',
          type: 'column',
          name: l10n('toolkit.debts', 'Her-Debts'),
        ...
        },
        {
          id: 'assets',
          type: 'column',
          name: l10n('toolkit.assets', 'Her-Assets'),
        ...
        },
        {
          id: 'networth',
          type: 'area',
          name: l10n('toolkit.netWorth', 'Her-Net Worth'),
  ...
  _calculateData() {
    ...
    const allToolData = { assets: [], labels: [], debts: [], netWorths: [] };
    ...
    function pushCurrentAccountData() {
      ...
      allToolData.assets.push(assets);
      allToolData.debts.push(debts);
      allToolData.netWorths.push(assets - debts);
      allToolData.labels.push(localizedMonthAndYear(lastMonth));
    }
    ...
    if (
      lastMonth &&
      allToolData.labels[allToolData.labels.length - 1] !== localizedMonthAndYear(lastMonth)
    ) {
      pushCurrentAccountData();
    }
    ...
    if (transactions.length) {
      ...
      while (transactionMonth.isBefore(lastFilterMonth)) {
        if (!allToolData.labels.includes(localizedMonthAndYear(transactionMonth))) {
          const { assets, debts, netWorths, labels } = allToolData;
    ...
    const { labels, assets, debts, netWorths } = allToolData;
